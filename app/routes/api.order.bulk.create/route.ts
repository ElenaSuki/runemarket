...
export const action: ActionFunction = async ({ request }) => {
  try {
    ...
    for (let i = 0; i < psbt.txInputs.length; i++) {
      ...
      const inscriptionMatch = offers.find(
        (offer) =>
          offer.inscription_txid === txid && offer.inscription_vout === index,
      );

      if (inscriptionMatch) {
        const offerPsbt = Psbt.fromHex(inscriptionMatch.psbt);

        if (
          !offerPsbt.data.inputs[0].finalScriptWitness &&
          !offerPsbt.data.inputs[0].finalScriptSig
        ) {
          offerPsbt.finalizeInput(0);
        }

        if (offerPsbt.data.inputs[0].finalScriptSig) {
          psbt.updateInput(i, {
            finalScriptSig: offerPsbt.data.inputs[0].finalScriptSig,
          });
        } else if (offerPsbt.data.inputs[0].finalScriptWitness) {
          psbt.updateInput(i, {
            finalScriptWitness: offerPsbt.data.inputs[0].finalScriptWitness,
          });
        }
      ...
      const runeMatch = offers.find(
        (offer) =>
          offer.location_txid === txid && offer.location_vout === index,
      );

      if (runeMatch) {
        const offerPsbt = Psbt.fromHex(runeMatch.psbt);

        if (
          !offerPsbt.data.inputs[1].finalScriptWitness &&
          !offerPsbt.data.inputs[1].finalScriptSig
        ) {
          offerPsbt.finalizeInput(1);
        }

        if (offerPsbt.data.inputs[1].finalScriptSig) {
          psbt.updateInput(i, {
            finalScriptSig: offerPsbt.data.inputs[1].finalScriptSig,
          });
        } else if (offerPsbt.data.inputs[1].finalScriptWitness) {
          psbt.updateInput(i, {
            finalScriptWitness: offerPsbt.data.inputs[1].finalScriptWitness,
          });
        }
      ...
      return json(errorResponse(30017));
    }
